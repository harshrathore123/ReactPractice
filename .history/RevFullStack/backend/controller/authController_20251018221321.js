...
const TravelUser = (req,res)=>{
    ...
    if(!firstname||!lastname||!email||!password||!confirm_password||!age||!city){
        res.json({msg:"All field is required"})
    }

    if(password!==confirm_password){
        res.json({msg:"Password and Confirm Password not matched"})
    }

    bcrypt.hash(password,10,(err,hash)=>{
        if(err) res.json({msg:`Password not hashing ${err.message}`})
        else{
            const insertQuery = 'insert into TravelUser (firstname,lastname,email,password,city,age)';
...
module.exports = {Booking,TravelUser};